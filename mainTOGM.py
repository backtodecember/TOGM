...
class Robosimian(System):
	def __init__(self):
		...
		self.robot = robosimianSimulator(q= q_2D,q_dot = q_dot_2D,dt = 0.005,solver = 'cvxpy')
...
problem.preProcess()
cfg = OptConfig(backend='snopt', print_file='tmp.out', print_level = 1, opt_tol = 1e-4, fea_tol = 1e-4)
