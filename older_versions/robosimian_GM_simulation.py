	...
	def simulateOnce(self,u):#debug,counter):
		...
		if self.formulation == 'H':
			...
			if NofContacts > 0:
				...
				#  	dhy_bar[i+16,:] = dhy_bar[i+16,:]*gammas[i,0]
				...
				# 	G[i,i] = constraint_values[i,0]
				# for i in range(len(gammas)):
				# 	self.dhx[16+i,:] = gammas[i,0]*self.dhx[16+i,:]


				#Here
				F1[self.Dwc + self.Dlambda+12:self.Dwc + self.Dlambda+120,self.Dwc + self.Dlambda+12:self.Dwc + self.Dlambda+120] = G
				#F1[self.Dwc + self.Dlambda+16:self.Dwc + self.Dlambda+120,self.Dwc + self.Dlambda+16:self.Dwc + self.Dlambda+120] = G
...
if __name__=="__main__":
	...
	q_2D = np.array([0.0,0.944,0.0] + [0.6- 1.5708,0.0,-0.6]+[0.6+1.5708,0.0,-0.6]+[0.6-1.5708,0.0,-0.6] \
	  	+[0.6+1.5708,0.0,-0.6])[np.newaxis] #four feet on the ground at the same time 
